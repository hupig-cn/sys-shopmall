...
<<<<<<< HEAD
    ...
    List<Map<String,Object>> findAllProduct(int pageNum, Integer pageSize,String content);
=======
    @Query(value = "SELECT c.id as commodityid,c.name as name ,s.fileid as fileid,c.postage as postage,s.id as id,s.model as model,s.specifications as specifications ,s.price as price,c.salevalue as salevalue,s.discount as discount from commodity c LEFT JOIN specifications s ON s.commodityid = c. ORDER BY s.other DESC, c.createdate DESC LIMIT ?1,?2", nativeQuery = true)
    List<Map<String,Object>> findAllProduct(int pageNum, Integer pageSize);
>>>>>>> branch 'code' of git@github.com:hupig-cn/sys-shopmall.git
