...
public interface Rewrite_CommodityRepository extends JpaRepository<Commodity, Long> {
    ...
    @Query(value = "SELECT c.id as commodityid,c.name as name ,s.fileid as fileid,c.postage as postage,s.id as id,s.model as model,s.specifications as specifications ,s.price as price,c.salevalue as salevalue,s.discount as discount from commodity c LEFT JOIN specifications s ON s.commodityid = c.id where name like %?1% or model like %?1% or specifications like %?1% ORDER BY s.other DESC, c.createdate DESC LIMIT ?2,?3", nativeQuery = true)
    ...
    //hui
    @Query(value = "select id from commodity Where classificationid = ?1",nativeQuery = true)
    List<Long> findCommodityByClassificationid(String classificationid);
}
