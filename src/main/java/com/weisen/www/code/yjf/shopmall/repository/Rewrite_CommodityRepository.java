...
public interface Rewrite_CommodityRepository extends JpaRepository<Commodity, Long> {
    ...
    @Query(value = "SELECT c.id as commodityid,c.name as name ,s.fileid as fileid,c.postage as postage,s.id as id,s.model as model,s.specifications as specifications ,s.price as price,c.salevalue as salevalue,s.discount as discount from commodity c LEFT JOIN specifications s ON s.commodityid = c.id  ORDER BY c.createdate DESC LIMIT ?1,?2", nativeQuery = true)
